...
variable "topic-name" {
  type        = string
  default     = "sentinel-GCPCloudAudit-topic"
...
resource "google_pubsub_topic" "sentinel-GCPCloudAudit-topic" {
  count   = "${var.topic-name != "sentinel-GCPCloudAudit-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
...
resource "google_pubsub_subscription" "sentinel-subscription" {
  project = data.google_project.project.project_id
  name    = "sentinel-subscription-GCPCloudAuditlogs"
  topic   = var.topic-name
  depends_on = [google_pubsub_topic.sentinel-GCPCloudAudit-topic]
}

resource "google_logging_project_sink" "sentinel-sink" {
  project    = data.google_project.project.project_id
  count      = var.organization-id == "" ? 1 : 0
  name       = "GCPCloudAudit-logs-sentinel-sink"
...
resource "google_logging_organization_sink" "sentinel-organization-sink" {
  ...
  name   = "GCPCloudAudit-logs-organization-sentinel-sink"
